...
import { categories, projects, CategoryKey } from "./data";
import ProjectCarousel from "./ProjectCarousel";

export default function ProjectsGrid() {
  // ✅ IMPORTANT FIX
  const [filter, setFilter] = React.useState<CategoryKey>("all");
  const [activeImages, setActiveImages] = React.useState<string[] | null>(null);
      ...
      <section id="projects" className="mx-auto max-w-7xl px-6 py-14">
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p, i) => (
            <motion.div
              key={p.title}
              onClick={() => setActiveImages(p.images)}
              ...
              className="group relative cursor-pointer overflow-hidden rounded-2xl border border-white/30 bg-gray-800 shadow-lg backdrop-blur-xl"
              style={
                {
                  "--mx": "50%",
                  "--my": "30%",
